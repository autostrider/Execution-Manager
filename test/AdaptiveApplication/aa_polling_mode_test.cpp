...
protected:
    ...
    AdaptiveApp app{std::move(factoryMock), std::move(stateClientMock),
                    std::move(compClientMock), std::move(meanCalculatorMock)};
...
{
    {
        ...
        expectGetComponentState(expectedStateKOn, ComponentClientReturnType::kSuccess);
        ...
        expectConfirmComponentState(expectedStateKOn, ComponentClientReturnType::kSuccess);
...
TEST_F(AppPollingModeTest, Should_StayInKOnWhenPerformActionCalled)
{
    goToInit();
    expectGetComponentState(expectedStateKOn, ComponentClientReturnType::kSuccess);
    expectConfirmComponentState(expectedStateKOn, ComponentClientReturnType::kUnchanged);
...
TEST_F(AppPollingModeTest, shouldReportErrorWhenErrorOccuredWhileGettingCompState)
{
    ...
    expectGetComponentState(expectedStateKOn, ComponentClientReturnType::kGeneralError);
    expectConfirmComponentState(expectedStateKOn, ComponentClientReturnType::kGeneralError);
...
TEST_F(AppPollingModeTest, shouldStayInKOnStateWhenInvalidStateReceived)
{
    ...
    expectGetComponentState(expectedStateInvalid, ComponentClientReturnType::kSuccess);
    expectConfirmComponentState(expectedStateInvalid, ComponentClientReturnType::kInvalid);
    performActionOnState(stateRunMockPtr);
}
...
TEST_F(AppPollingModeTest, shouldStayInKOnStateWhenGetComponentStateFailed)
{
    ...
    expectGetComponentState(expectedStateInvalid, ComponentClientReturnType::kGeneralError);
    expectConfirmComponentState(expectedStateInvalid, ComponentClientReturnType::kGeneralError);
    performActionOnState(stateRunMockPtr);
}
...
TEST_F(AppPollingModeTest, shouldTransitToKOffStateWhenPerformActionCalled)
{
    ...
    {
        InSequence seq;
        expectGetComponentState(expectedStateKOff, ComponentClientReturnType::kSuccess);
        expectConfirmComponentState(expectedStateKOff, ComponentClientReturnType::kSuccess);
    }
