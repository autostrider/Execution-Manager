...
void Server::onRunning() {
    ...
    while (m_isRunning) {
        ...
        if (result > 0) {
            auto clientConnection = std::make_unique<Client>(SOCKET_SERVER_PATH, std::move(std::make_unique<ClientSocket>()));
            clientConnection->setClientFd(result);
            clientConnection->setConnected(true);
            std::lock_guard<std::mutex> guard(m_mtx1);
            m_activeConnections.push_back(std::move(clientConnection));
        } else {
            continue;
        }
    }
